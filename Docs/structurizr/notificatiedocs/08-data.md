...
> Stand van de implementatie: het huidige NMC (PoC-fase) heeft de tabellen `notificatie` (status, reden, versie, `callback_url`, `laatste_status_update` en de niet meer gebruikte kolom `laatste_status`), `poging`, `event` (gepartitioneerd op transactie-id) en `toegestane_overgang`, met het statusmodel hierboven en de databasetrigger die elke nieuwe versie aan een event koppelt (`V4` tot en met `V6`). Het oude model met `notificatie_status` en `external_reference` is vervallen. Nog niet gebouwd zijn de tabellen `dienstverlener`, `taak`, `verzendbudget`, `bevestiging` en `webhookpositie`, en de overige kolommen van `notificatie` uit de tabel hierboven. Een nachtelijke retentiejob verwijdert notificaties waarvan de laatste statuswijziging ouder is dan de bewaartermijn, ongeacht de status.
