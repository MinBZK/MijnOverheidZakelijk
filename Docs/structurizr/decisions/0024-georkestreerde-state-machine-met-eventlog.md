...
De paden dragen een major-versie: `POST /api/nmc/v1/centraal/notificaties`, `POST /api/nmc/v1/decentraal/notificaties`, `GET /api/nmc/v1/notificaties/{id}`, `GET /api/nmc/v1/notificaties?dvRef=` (een lijst), `DELETE /api/nmc/v1/notificaties/{id}` (annuleren), `GET /api/nmc/v1/notificaties/wijzigingen?cursor=`, `PUT /api/nmc/v1/notificaties/wijzigingen/bevestiging`, en de receipt-callback `POST /api/nmc/v1/notifynl-callback`. Fouten zijn RFC 9457-problemen met een eigen type voor het quotum (429) en voor een vervallen cursor (410).
...
Lezen van de eigen status, met versie, op notificatie-id of via `GET /api/nmc/v1/notificaties?dvRef=` op de `dvRef`-HMAC binnen de eigen DV.
