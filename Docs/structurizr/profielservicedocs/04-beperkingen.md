...
- Service-to-service-toegang verloopt via een API-gateway op basis van FSC (contracten) en FTV (tokens) uit het Federatief Datastelsel. Opgelegd door stelselafspraken. Impact: aanroeper-autorisatie (de organisatie achter de aanroep) is een infrastructurele en contractuele zaak; de Profiel Service voert daar geen eigen scope-mapping op uit. De service leidt de handelende dienstverlener af uit de OIN in de FTV-laag; de gateway geeft het gevalideerde FTV-token door (voorgeschreven door fsc-core §4.7.1.3) en de service hervalideert het. Voor de eindgebruikersidentiteit valideert de Profiel Service per request wél een meegestuurd subject-JWT (uitgegeven door DigiD of de eHerkenning-makelaar) tegen de JWKS van de uitgevende identity provider. Impact: de service kent een trustconfiguratie met geaccepteerde `iss`-waarden en JWKS-URLs, en is afhankelijk van de beschikbaarheid van die JWKS-endpoints. Zie [§09 Infrastructuur, Authenticatie en autorisatie](09-infrastructuur-architectuur.md#authenticatie-en-autorisatie) en de [bijlage Logging, Toegang en Doelbinding](../decisions/addendum/profiel-service-logging-en-toegang.md).
- Scope-gebaseerde autorisatie voor dienstverleners via FDS; scopes beperken toegang tot de minimaal noodzakelijke rechten (least privilege), bijvoorbeeld read-only waar van toepassing. Opgelegd door architectuurkader; scopes worden in FSC-contracten vastgelegd en op de gateway afgedwongen.
